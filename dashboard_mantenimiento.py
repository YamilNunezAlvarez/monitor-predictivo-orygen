...
h1, h2, h3 { font-family: 'Share Tech Mono', monospace; color: #ffffff; }
...
# ─── SIDEBAR ──────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<h1 style='font-family:Share Tech Mono,monospace;color:#ffffff;font-size:2rem;margin-bottom:0'>⚡ ORYGEN</h1>", unsafe_allow_html=True)
    st.markdown("<div class='seccion-titulo' style='margin-top:16px'>Cargar datos</div>", unsafe_allow_html=True)

    archivo = st.file_uploader("CSV de datos (vibracion)", type=["csv"])

    st.markdown("---")
    st.markdown(f"""
    <div style='font-family:monospace; font-size:0.78rem; color:#a0b4cc; line-height:2'>
        <div style='color:#3b82f6;letter-spacing:2px;font-size:0.7rem'>EQUIPO</div>
        <span style='color:#ffffff'>{equipo} · {cojinete}</span>
        <br>
        <div style='color:#3b82f6;letter-spacing:2px;font-size:0.7rem;margin-top:10px'>FECHA DE ANÁLISIS</div>
        <span style='color:#ffffff'>{fecha_sim.strftime('%d/%m/%Y')} (hoy)</span>
    </div>
    """, unsafe_allow_html=True)

# ─── MAIN ─────────────────────────────────────────────────────────────────────
st.markdown("# <span style='color:#ffffff'>⚡ MONITOR PREDICTIVO DE MANTENIMIENTO</span>", unsafe_allow_html=True)
st.markdown(f"<span style='color:#a0b4cc;font-size:0.85rem;font-family:monospace'>{equipo} · {cojinete}</span>", unsafe_allow_html=True)
st.markdown("---")
...
with st.spinner("Procesando datos..."):
    ...
    modelo_if = joblib.load('modelo_coj_g1.pkl')
