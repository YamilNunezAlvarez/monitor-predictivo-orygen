...
with st.sidebar:
    st.markdown("## ⚡ ORYGEN")
    st.markdown("<div class='seccion-titulo'>Configuración</div>", unsafe_allow_html=True)

    archivo = st.file_uploader("Cargar CSV de datos", type=["csv"])

    modelo_archivo = st.file_uploader("Cargar modelo (.pkl)", type=["pkl"])

    st.markdown("---")
    st.markdown("<div class='seccion-titulo'>Parámetros</div>", unsafe_allow_html=True)
    equipo    = st.text_input("Equipo",   value="Turbina G1")
    cojinete  = st.text_input("Cojinete", value="COJ-G1-1")
    filtro_min = st.slider("Filtro confirmación (min)", 30, 360, 120, 30)
    contamination = st.select_slider("Contamination IF", [0.001, 0.003, 0.005, 0.01, 0.02], value=0.003)

    st.markdown("---")
    fecha_analisis = st.date_input("Fecha de análisis", value=pd.Timestamp('2026-03-13'))
...
with st.spinner("Procesando datos..."):
    ...
    fecha_sim    = pd.Timestamp(fecha_analisis)
